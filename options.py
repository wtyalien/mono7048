...
class LiteMonoOptions:
    def __init__(self):
        ...
        #wty
        self.parser.add_argument("--Res_Version",
                                 type=str,
                                 help="number of resnet layers",
                                 default="ResNet50_Weights.IMAGENET1K_V2.url",
                                 choices=["ResNet50_Weights.IMAGENET1K_V2.url","ResNet101_Weights.IMAGENET1K_V2.url"]
                                   )
...
#                                  | 位置    | 取值     | 所属网络     | 含义                   | 实际作用                             |
# | ----- | ------ | -------- | -------------------- | -------------------------------- |
# | **0** | 0.0001 | DepthNet | 初始学习率                | 训练第一天从 1e-4 开始                   |
# | **1** | 5e-6   | DepthNet | **最低**学习率            | cosine 退火不会低于 5e-6               |
# | **2** | 31     | DepthNet | 第一个 cosine **半周期长度** | 31 epoch 完成一次 1e-4 → 5e-6 → 1e-4 |
# | **3** | 0.0001 | PoseNet  | 初始学习率                | Pose 分支与 Depth 初始值相同             |
# | **4** | 1e-5   | PoseNet  | **最低**学习率            | 比 Depth 稍高，防止 Pose 过慢            |
# | **5** | 31     | PoseNet  | 第一个 cosine **半周期长度** | 同样 31 epoch 一循环                  |
        ...
        self.parser.add_argument("--use_diffusion",
                                 help="if set, uses diffusion depth refinement",
                                 action="store_true")
        self.parser.add_argument("--diffusion_channels",
                                 type=int,
                                 default=48,  # 改为48节省显存
                                 help="diffusion module feature channels")
        self.parser.add_argument("--diffusion_steps",
                                 type=int,
                                 default=4,  # 改为4加速推理
                                 help="diffusion inference steps")
        self.parser.add_argument("--diffusion_loss_weight",
                                 type=float,
